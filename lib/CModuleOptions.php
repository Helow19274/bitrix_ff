...
class CModuleOptions
{
	...
	public function __construct($module_id, $arTabs, $arGroups, $arOptions, $need_access_tab = false)
	{
		...
		if($need_access_tab)
			$this->arTabs[] = array(
				'DIV' => 'edit_access_tab',
				'TAB' => GetMessage("IQCREATIVE_ORDERADMIN_PRAVA_DOSTUPA"),
				'ICON' => '',
				'TITLE' => GetMessage("IQCREATIVE_ORDERADMIN_NASTROYKA_PRAV_DOSTU")
			);
			...
			foreach($this->arOptions as $option => $arOptParams)
			{
				...
				switch($arOptParams['TYPE'])
				{
					...
					case 'TEXT':
						...
						break;
					case 'SELECT':
						if($arOptParams['ALLOW_EMPTY'] == 'Y') {
							array_unshift($arOptParams['VALUES']['REFERENCE_ID'], 0);
							array_unshift($arOptParams['VALUES']['REFERENCE'], 'Выбрать...');
						}
						$input = SelectBoxFromArray($opt, $arOptParams['VALUES'], $val, '', '', $arOptParams['REFRESH'] == 'Y', ($arOptParams['REFRESH'] == 'Y' ? $this->module_id : ''));
						if($arOptParams['REFRESH'] == 'Y')
							$input .= '<input type="submit" name="refresh" value="OK" />';
						break;
					case 'MSELECT':
                    ...
                    case 'MCHECKBOX':
                        ...
                        foreach($arOptParams['VALUES']['REFERENCE_ID'] as $key => $value) {
                            $input .= '<p style="margin-top:0;"><input type="checkbox" name="' . $opt . '[]" value="' . $arOptParams['VALUES']['REFERENCE_ID'][$key] . '"' . (!empty($val) && array_search($arOptParams['VALUES']['REFERENCE_ID'][$key], $val) !== false ? ' checked' : '') . ' /> - ' . $arOptParams['VALUES']['REFERENCE'][$key] . '</p>';
                        }
                        break;
					case 'COLORPICKER':
						...
						$APPLICATION->IncludeComponent('bitrix:main.colorpicker', '', Array(
								'SHOW_BUTTON' => 'Y',
								'ID' => $opt,
								'NAME' => GetMessage("IQCREATIVE_ORDERADMIN_VYBOR_CVETA"),
